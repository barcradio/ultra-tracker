...
Write-Host "`n[1/4] Enforcing terminal security shield..." -ForegroundColor Yellow
...
Write-Host "`n[2/4] Purging old caches and running secure installation..." -ForegroundColor Yellow
if (Test-Path .\node_modules) {
...
Write-Host "`n[3/4] Updating target browserslist definitions..." -ForegroundColor Yellow
npx update-browserslist-db@latest --yes

# Step 4: Manually extract framework binaries and recompile C++ components
Write-Host "`n[4/4] Executing isolated platform binary extraction and compilation..." -ForegroundColor Yellow
node .\node_modules\electron\install.js
pnpm dlx @electron/rebuild --only better-sqlite3

Remove-Item Env:\npm_config_ignore_scripts -ErrorAction SilentlyContinue
Write-Host "Development hydration complete. Start the app with 'pnpm dev' or 'pnpm start'." -ForegroundColor Green